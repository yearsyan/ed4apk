...
```sh
java -jar aqe.jar examples files --file patch.json > patch.json
java -jar aqe.jar examples files --file config.json > config.json
java -jar aqe.jar examples files --file new.txt > new.txt
...
最简单的非批处理写法为 `java -jar aqe.jar replace app.apk assets/config.json=config.json -o edited.apk`；
