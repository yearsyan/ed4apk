...
此流程的 javac 步骤需要 JDK；ed4apk 不接受 .java。示例是纯 Java，不依赖 Android API。
前置条件：app.apk 内有 classes.dex、com.example.Main，没有 com.example.ed4apk.Helper。
按真实 APK 修改调用方类名，所有路径在同一工作目录下。

```sh
java -jar ed4apk.jar examples java-class --file Helper.java > Helper.java
javac --release 8 -d classes Helper.java
java -jar ed4apk.jar dex export app.apk com.example.Main -o Main.smali
java -jar ed4apk.jar examples java-class --file patch.json > patch.json
```
