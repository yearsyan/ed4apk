...
tasks.test {
    ...
    systemProperty("aqe.jar", layout.buildDirectory.file("libs/aqe.jar").get().asFile.absolutePath)
    providers.gradleProperty("aqeTestJava").orNull?.let { systemProperty("aqe.java", it) }
}

tasks.shadowJar {
    archiveFileName = "aqe.jar"
    mergeServiceFiles()
    filesMatching("META-INF/services/**") {
        duplicatesStrategy = DuplicatesStrategy.INCLUDE
    }
    exclude("META-INF/*.SF", "META-INF/*.RSA", "META-INF/*.DSA", "module-info.class")
    manifest.attributes["Main-Class"] = "dev.aqe.Main"
